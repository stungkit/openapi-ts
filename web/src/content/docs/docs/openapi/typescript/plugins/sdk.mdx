...
Enabling examples does not produce visible output on its own. Examples are written into the source specification and can be consumed by documentation tools. To persist that specification, enable [Source](/docs/openapi/typescript/configuration/output#source) generation.
